...
#define GEO_COMMON_HEADER_LEN 16
//#define GEO_SECURED_HEADER_LEN 30
...
{
    printk(KERN_INFO "now execute geo_extend_header\n");
		int found;
		struct geohdr_p *tmp;
		tmp = ghdr;
...
int gn_data_indication(struct sk_buff *skb, struct geohdr_basic *gbhdr, struct geohdr_common *gchdr, int type)
{
    ...
    switch(type){
        ...
        case BEACON:
            {
                ...
                printk(KERN_INFO "GN_indication for BEACOM\n");
                //set source position vector
                gn_indication_params.so_pv = gehdr->so_pv;
        ...
        case LSREQ:
            {
                printk(KERN_INFO "GN_indication for LSREQ\n");
        ...
        case LSREP:
            {
                printk(KERN_INFO "GN_indication for LSREP\n");
